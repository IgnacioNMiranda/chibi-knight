...
import { getNextAvailableRoleFromUser, getRole, getRoleFromUser, roles, utilLinks, CustomPrecondition } from '@/utils'
...
export class MyRoleCommand extends Command {
  ...
  async messageRun(message: Message): Promise<Message> {
    ...
    if ((nextAvailableRole && nextAvailableRole.name !== roles.zote.name) || !nextAvailableRole) {
      embedMessage.addField('You have the following role:', `• ${currentBotRelatedRole.name}`)
    ...
    if (nextAvailableRole) {
      embedMessage.setFooter(
        `You need ${nextAvailableRole.requiredPoints - parseInt(score)} more points to get '${
          nextAvailableRole.name
        }' role.`
      )
    } else {
      embedMessage.setFooter(`You are ${currentBotRelatedRole.name}, you have reached the absolute supremacy!!`)
    }
